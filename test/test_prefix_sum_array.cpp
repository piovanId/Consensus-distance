    ...
    protected:
        ...
        PrefixSumArraysTest() {
            ...
            for (int i = 0; i < gfa_files_paths.size(); ++i) {
                ...
                graph.reset(new gbwtgraph::GBWTGraph(*(gfa_parse.first),
                                                     *(gfa_parse.second)));
...
/*
    ...
    TEST_F(PrefixSumArraysTest, distanza) {
        for (int i = 0; i < prefix_sums_arrays->size(); ++i) {

            ASSERT_EQ(0,  (*prefix_sums_arrays)[i]->get_distance_between_positions_in_path(0,0,0));
            if(i>1)
                std::cout<< (*prefix_sums_arrays)[i]->get_distance_between_positions_in_path(0,2,0);
